...
class CSDBinarizer(OpencpopBinarizer):

    def load_meta_data(self):
        raw_data_dir = hparams['raw_data_dir']
        utterance_labels = open(os.path.join(raw_data_dir, 'transcriptions_complete.txt')).readlines()

        for utterance_label in utterance_labels:
            song_info = utterance_label.split('|')
            item_name = raw_item_name = song_info[0]
            item_song = item_name.split('_')[0]
            self.item2wavfn[item_name] = f'{raw_data_dir}/segments/{item_song}/{item_name}.wav'
            self.item2txt[item_name] = song_info[1]
            self.item2ph[item_name] = song_info[2]
            self.item2wdb[item_name] = [0] * len(song_info[2].split(' '))
            accum = 0
            for syllable in song_info[1].split(' '):
                syllable = syllable.split('_')
                self.item2wdb[item_name][accum + len(syllable) - 1] = 1
                accum += len(syllable)

            self.item2ph_durs[item_name] = [float(x) for x in song_info[5].split(" ")]
            self.item2midi[item_name] = [int(x) if x != 'rest' else 0 for x in song_info[3].split(" ")]
            self.item2midi_dur[item_name] = [float(x) for x in song_info[4].split(" ")]
            self.item2is_slur[item_name] = [int(x) for x in song_info[6].split(" ")]
            self.item2spk[item_name] = 'csd_en'
