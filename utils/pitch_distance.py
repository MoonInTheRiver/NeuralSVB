...
def align_from_distances(distance_matrix, debug=False, return_mindist=False):
    ...
    if debug:
        ...
        plt.show()
    if return_mindist:
        return results, dtw[-1, -1], path_length
    return results
...
def LoNDTWDistance(src, tgt):
    ...
    alignment, min_distance, path_length = align_from_distances(costs.T.cpu().detach().numpy(), return_mindist=True)  # [T]
    return alignment, min_distance, path_length
...
#     print(min_distance)
# python utils/pitch_distance.py --config egs/datasets/audio/PopBuTFy/svc_ppg.yaml