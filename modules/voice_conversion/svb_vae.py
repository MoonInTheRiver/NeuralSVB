...
class AttnMleSVBVAE(MleSVBVAE):
    def __init__(self, dict_size):
        ...
        self.embed_positions = SinusoidalPositionalEmbedding(
            ...
            init_size=hparams['max_frames'] + self.padding_idx + 1,
        )
    ...
    def get_aligned_ppg(self, src_ppg, src_mel, alignment):
        ...
        q_tgt = gathered_ppg + self.embed_positions(gathered_ppg.abs().sum(-1))  # [B, T, H]
        k_src = src_ppg + self.embed_positions(src_ppg.abs().sum(-1))   # [B, S, H]
        attn, attn_weights = self.ref_attn(q_tgt.transpose(0, 1), k_src.transpose(0, 1),
                                src_ppg.transpose(0, 1), key_padding_mask=1.0-src_non_padding.squeeze(-1), attn_mask=None)
        return attn.transpose(0, 1), attn_weights
    ...
    def forward(self, amateur_mel=None, prof_mel=None,
        ...
        if 'a2p' in concurrent_ways:
            ...
            # align
            fake_prof_ppg, attn_weights = self.get_aligned_ppg(amateur_conds['h_content'], amateur_mel, a2p_alignment, amateur_conds['tgt_nonpadding'])
            a2p_cond_sum = self.encoded_embed_proj(torch.cat([
                                prof_conds['h_pitch'],
                                fake_prof_ppg,
                                amateur_conds['h_style'][:, :1, :].repeat(1, prof_conds['h_pitch'].shape[1], 1)
                                ], -1)).transpose(1, 2)  # [B, H, T]
            a2p_out['attn'] = attn_weights
            # recon a2p from mapped z
            a2p_out['mel_out'] = self.vae_model.decoder(mapped_amatuer_z_q, prof_conds['tgt_nonpadding'].transpose(1, 2), g=a2p_cond_sum).transpose(1, 2)
